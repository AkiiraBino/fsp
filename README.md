...
## Реализация вычисления для прямых маршрутов из города `city` разницы расстояний между ближайшими по алфавиту городами.

Использованы CTE и оконные функции. SQL захардкожен, при этом неверно с точки зрения архитектуры. Причина: нехватка времени по личным обстоятельствам.
В правильной реализации необходимо создать третий репозиторий, который будет работать с 2 таблицами и позволит вынести эту логику туда, как и логику с алгоритмом Дейкстры, которую можно было упростить засчет определения в репозитории соответствующих функций для работы с 2 таблицами, но осознание пришло слишком поздно
